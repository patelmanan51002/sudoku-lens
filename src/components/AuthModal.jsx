...
export default function AuthModal({ isOpen, onClose, onAuthSuccess }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-3 sm:p-4 overflow-y-auto">
      <div className="bg-white rounded-3xl p-5 sm:p-8 max-w-md w-full max-h-[92vh] overflow-y-auto shadow-2xl border border-slate-100 animate-pop relative my-auto">
        <button
          onClick={onClose}
          ...
          <div className="flex p-1 bg-slate-100 rounded-xl mb-5 text-xs font-bold">
            <button
              ...
              className={`flex-1 py-1.5 rounded-lg transition-all ${
                activeMode === 'signin' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-900'
              }`}
            >
              Sign In
            </button>
